...
class pepperEnv3(gym.Env):

    def __init__(self, render = False, obx = 1, oby = 1):

        super(pepperEnv3, self).__init__()
        self.obx = obx
        self.oby = oby
        self.render = render
        ...
        self.client3 = self.simulation_manager.launchSimulation(gui=self.render, use_shared_memory=True)
        self.pepper = self.simulation_manager.spawnPepper(self.client3, spawn_ground_plane=True)
    ...
    def step(self, action):
        ...
        if d < 1:
            ...
            print('3 LLego al objetivo')

        elif min(self.left) < 0.5 or min(self.right) < 0.5 or min(self.front) < 0.5:
            reward = -5
            print('3 Se choco con un obstaculo')

        elif d >= 10 or (self.steps > 250 and d >= 10):
            reward = -4
            print('3 Se fue muy lejos')

        elif self.steps > 250:
            reward = -1-(2*d)/10
            print('3 No llego a ningun lado')
    ...
    def reset(self):
        ...
        self.simulation_manager.resetSimulation(self.client3)
        self.pepper = self.simulation_manager.spawnPepper(self.client3, spawn_ground_plane=True)
    ...
    def close(self):
        ...
        #Parar simulacion 
        self.simulation_manager.stopSimulation(self.client3)